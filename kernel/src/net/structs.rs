...
impl UdpSocketState{
    pub fn new()-> Self {
        let rx_buffer  = UdpSocketStateBuffer::new(
            vec![UdpPacketMatadata::EMPTY;UDP_METADATA_BUF],
            vec![0;UDP_RECVBUF],
        );
        let tx_buffer = UdpSocketStateBuffer::new(
            vec![UdpPacketMatadata::EMPTY;UDP_METADATA_BUF],
            vec![0;UDP_SENDBUF],
        );
        let socket = UdpSocket::new(rx_buffer,tx_buffer);
        let hander = GlobalSocketHandle(SOCKETS.lock().add(socket));
...
#[repr(C)]
struct ArpReq{
    arq_pa: SockAddrPlaceholder,
    arq_ha: SockAddrPlaceholder,
    arq_flags: u32,
...
impl RawSocketState{
    pub fn new(protocol : u8) -> Self{
        let rx_buffer  =RawSocketState::new(
            vec![RawPacketMetadata::EMPTY;RAW_METADATA_BUF],
            vec![0;RAW_RECVBUF],
        );
        let tx_buffer = RawSocketState::new(
            vec![RawPacketMetadata::EMPTY;RAW_METADATA_BUF],
            vec![0;RAW_SENDBUF],
        );
        let socket = RawSocket::new(
            IpVersion::Ipv4,
            IpProtocol::form(protocol),
            rx_buffer,
            tx_buffer,
        );
        let handle = GlobalSocketHandle(SOCKETS.lock().add(socket));
...
enum_with_unkonw!{
    ...
    pub doc enum NetlinkMessageType{
        /// Nothing 
        Noop  = 1,
        /// Error 
        Error = 2,
